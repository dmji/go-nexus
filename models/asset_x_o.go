...
type AssetXO struct {
	...
	// blob created
	// Format: date-time
	BlobCreated strfmt.DateTime `json:"blobCreated,omitempty"`
...
func (m *AssetXO) Validate(formats strfmt.Registry) error {
	...
	if err := m.validateBlobCreated(formats); err != nil {
		res = append(res, err)
	}
...
func (m *AssetXO) validateBlobCreated(formats strfmt.Registry) error {
	if swag.IsZero(m.BlobCreated) { // not required
		return nil
	}

	if err := validate.FormatOf("blobCreated", "body", "date-time", m.BlobCreated.String(), formats); err != nil {
		return err
	}
